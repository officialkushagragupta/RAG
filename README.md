...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="frontend/assets/logo_full.png">
  <img src="logo_full_dark.png" alt="RAG Document Chatbot" width="200">
</picture>
...
- **Rich metadata flows forward, no backward lookups.** A chunk is
  ...
  `Citation`, with no second database round-trip to "hydrate" the result.
...
Every simplification below was a conscious call to **optimize for a
...
for the lifetime of the running backend process. This is intentional, to
keep the app lightweight and deployable on free hosting tiers with no
database dependency.
...
**7. Unified logging & observability strategy.** Today logs are written
to rotating files on disk (`backend/logs/`), which is fine for a single
process but doesn't survive container restarts or aggregate across
replicas. Ship logs and traces to a centralized platform (for example
Azure Application Insights, or an ELK / Grafana Loki stack) for
searchable, correlated, per-request telemetry: latency percentiles,
error rates, retrieval quality, and live health, all in one dashboard
instead of `grep` over log files.
